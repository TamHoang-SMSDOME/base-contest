...
namespace BaseContest_WebForms.Models
{
    public class GenericFunctions : Constants
    {
        ...
        public int ChancesCalcs(BC_240801_COKESAVOURYOURZEN Entry)
        {
            if (Entry.IsValid)
            {
        ...
        /// </summary>
        public int ChancesCalcs(BC_240801_COKESAVOURYOURZEN Entry, int Cap)
        {
            if (Entry.IsValid)
            {
                //var chances = ((int)Math.Floor(Convert.ToDecimal(Entry.Amount) / Convert.ToInt32(TierAmount))) * Convert.ToInt32(TierChance);
        ...
        public FunctionResult SendBlobToAzureStorage(BC_240801_COKESAVOURYOURZEN Entry, HttpPostedFileBase file, string filename, out string AzureLink)
        {
            try
            {
        ...
        public FunctionResult SendByteToAzureStorage(BC_240801_COKESAVOURYOURZEN Entry, byte[] imageBytes, string filename, out string AzureLink)
        {
            try
            {
                //var b64s = base64String.ToString().Substring(base64String.ToString().IndexOf(',') + 1);
        ...
        public List<string> ExcludeHeaders(BC_240801_COKESAVOURYOURZEN query, List<string> Excludes)
        {
        ...
        public List<string> ExcludeHeaders(BC_240801_COKESAVOURYOURZEN_Winners query, List<string> Excludes)
        {
            ...
            var qu = query.GetType().GetProperties().Where(p => p.GetMethod.IsVirtual == false).Select(s => s.Name)
                                                      .Union(query.BC_240801_COKESAVOURYOURZEN.GetType().GetProperties().Where(p => p.GetMethod.IsVirtual == false).Select(s => s.Name))
        ...
        public List<Dictionary<string, object>> ConvertEntriesToDictionary(IEnumerable<BC_240801_COKESAVOURYOURZEN> ListOfEntries, List<string> ExcludedFields)
        {
            using (var db = new BaseEntities())
            {

                //var WinnerNRICs = db.BC_240801_COKESAVOURYOURZEN_Winners.Select(s => s.NRIC_NoPrefix).ToList();
                //var WinnerMobileNos = db.BC_240801_COKESAVOURYOURZEN_Winners.Select(s => s.MobileNo).ToList();
                ...
                return ListOfEntries.Select(s => s.GetType().GetProperties().Where(p => p.GetMethod.IsVirtual == false)
                                        ...
                                        .Union(new List<KeyValuePair<string, object>>() { new KeyValuePair<string, object>("ExcludePastWinner", s.BC_240801_COKESAVOURYOURZEN_Winners.Count > 0 ? false : true) }.AsEnumerable())
                                        //.Union(new List<KeyValuePair<string, object>>() { new KeyValuePair<string, object>("ExcludePastWinner", s.BC_240801_COKESAVOURYOURZEN_Winners.Count > 0 ? false : WinnerNRICs.Contains(s.NRIC_NoPrefix) ? false : true) }.AsEnumerable())
                                        //.Union(new List<KeyValuePair<string, object>>() { new KeyValuePair<string, object>("ExcludePastWinner", s.BC_240801_COKESAVOURYOURZEN_Winners.Count > 0 ? false : WinnerMobileNos.Contains(s.MobileNo) ? false : true) }.AsEnumerable())
        ...
        public List<Dictionary<string, object>> ConvertWinnersToDictionary(IEnumerable<BC_240801_COKESAVOURYOURZEN_Winners> ListOfWinners, List<string> ExcludedFields)
        {
            return ListOfWinners.Select(s =>
                                            ...
                                            .Union(s.BC_240801_COKESAVOURYOURZEN.GetType().GetProperties().Where(p => p.GetMethod.IsVirtual == false).ToDictionary(prop => prop.Name, val => val.GetValue(s.BC_240801_COKESAVOURYOURZEN, null)))
        ...
        /// </summary>
        public FunctionResult SendSms(BC_240801_COKESAVOURYOURZEN Entry, string respmessage)
        {
            if (AppID != null && AppSecret != null && AppID != "" && AppSecret != "" &&
